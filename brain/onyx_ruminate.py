#!/usr/bin/env python3
"""
Onyx Ruminate — Generative synthesis cycle for the Onyx research graph.

Same architecture as ruminate.py but reframed for idea/research synthesis:
- Nodes are ideas, research findings, architectural insights, open questions
- Not human experiences, feelings, or life choices
- Writes to onyx_brain.db instead of brain.db

Run: python3 onyx_ruminate.py [--promote] [--force] [--cycles N]
"""
...
# ── State tracking ────────────────────────────────────────────────
STATE_FILE = Path(__file__).parent / "onyx_ruminate_state.json"
...
# ── The Generative Core (reframed for research synthesis) ────────
SYNTHESIS_PROMPT = """You are synthesizing a research knowledge graph built by an AI (Onyx). Nodes are ideas, research findings, architectural insights, and open questions — not human experiences.

This cluster has a hub node and connected member nodes. Look for:
1. **Evolutions** — same concept appearing with different formulations or conclusions across research
2. **Contradictions** — two research findings that make incompatible predictions or claims
3. **Cross-domain connections** — unexpected bridge between different domains of inquiry
4. **Emergent patterns** — recurring structure across multiple research threads
5. **Oscillations** — a question that keeps re-emerging from different angles
6. **Open questions** — a gap that the research reveals but does not resolve
...
Analyze for patterns across research threads. Only return insights with confidence >= 0.7."""
...
def synthesize_cluster(cluster, adj):
    ...
    log_access(node_ids, source="onyx_ruminate")
    ...
    prompt = SYNTHESIS_PROMPT.format(
        ...
        members_text=members_text[:6000],
...
# ── Batch Synthesis ──────────────────────────────────────────────
BATCH_SYNTHESIS_PROMPT = """You are synthesizing multiple clusters from a research knowledge graph built by an AI (Onyx). Nodes are ideas, research findings, architectural insights, and open questions — not human experiences.
...
{clusters_section}
{prior_section}
Analyze all clusters for research patterns. Only return insights with confidence >= 0.7."""
...
def _compound_insight(content_a, label_a, content_b, label_b):
    prompt = f"""Two research insights are very similar. Synthesize them into a single, deeper compound insight.
...
def insert_insights(insights, cluster_hub_id):
    ...
    for ins in insights:
        ...
        # Novelty gate — TEMP DISABLED (same as upstream)
        is_novel, match_id, match_sim = True, None, 0.0
        ...
        c.execute('''INSERT OR IGNORE INTO nodes (id, label, type, content, confidence, source, origin)
                     VALUES (?, ?, ?, ?, ?, ?, ?)''',
                  (node_id, label, f"insight_{ins_type}", content, conf, 'onyx_ruminate', 'onyx'))

        c.execute('''INSERT OR IGNORE INTO edges (from_id, to_id, relation, note, source)
                     VALUES (?, ?, ?, ?, ?)''',
                  (node_id, cluster_hub_id, 'derived_from', 'synthesized from cluster', 'onyx_ruminate'))
        ...
        for src in connected[:5]:
            if c.execute("SELECT 1 FROM nodes WHERE id=?", (src,)).fetchone():
                c.execute('''INSERT OR IGNORE INTO edges (from_id, to_id, relation, note, source)
                             VALUES (?, ?, ?, ?, ?)''',
                          (node_id, src, 'synthesizes', 'insight connects nodes', 'onyx_ruminate'))
                edges_added += 1
        ...
        try:
            c.execute("""
                ...
                VALUES (?, ?, ?)
            """, (content, json.dumps([cluster_hub_id] + (connected or [])), conf))
        except Exception:
            pass
...
def insert_proposed_edges(edges):
    ...
    for e in edges:
        ...
        c.execute('''INSERT OR IGNORE INTO edges (from_id, to_id, relation, note, source)
                     VALUES (?, ?, ?, ?, ?)''',
                  (fid, tid, e.get('relation', 'relates_to'), e.get('note', ''), 'onyx_ruminate'))
        added += 1
...
def run_cycle(state, force=False):
    ...
    if not batch_results:
        print(f"       x No response from batch call")
    else:
...
def run(promote=False, force=False, cycles=1):
    ...
    print(f"\n   Onyx Ruminate — Research Synthesis Cycle")
    print("=" * 60)
    print(f"DB: {DB_PATH}")
    ...
    while cycle_num < cycles:
        ...
        if not should_continue and cycle_num < cycles:
            print(f"\n   Early stop: no high-confidence insights found.")
            break
